			...
			while (rs.next()) {
				...
				psAux = con.prepareStatement(
						"select dt.codigo AS codigo_detalle , dt.cabecera_pedido, pro.codigo_pro, pro.nombre, "
								...
								+ "Where dt.producto = pro.codigo_pro " + "AND pro.categoria_cat = c.codigo_cat "
				...
				while (rsAux.next()) {
					...
					int codigoCat = rsAux.getInt("categoria_cat");
		...
		try {
			...
			ps = con.prepareStatement(
					"select pro.codigo_pro, pro.nombre, udm.codigo_udm, udm.descripcion AS descripcion_udm, udm.categoria_udm, cast(pro.precio_de_venta as decimal(6,2)), "
							...
							+ "Where pro.categoria_cat = c.codigo_cat " + "AND pro.udm = udm.codigo_udm "
							+ "AND pro.codigo_pro = ?;");
			ps.setInt(1, codigo);
			rs = ps.executeQuery();
			if (rs.next()) {
				...
				int codigoCat = rs.getInt("categoria_cat");
